...
# * LOG_DIRS: where logs are stored (meant persistent logs that store messages and offsets)
...
if [ ! -z "$LOG_RETENTION_MSEC" ]; then
    echo "log retention msec: $LOG_RETENTION_MSEC"
    sed -r -i "s/(log.retention.msec)=(.*)/\1=$LOG_RETENTION_HOURS/g" $KAFKA_HOME/config/server.properties
fi
