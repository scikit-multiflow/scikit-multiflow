...
if [ $MODE = "--attach-tmux" ]; then
    if [ $PROFILE != "LOCAL" ]; then
        ...
        if [ $x -eq 0 ]; then
            ...
            if [ $PROFILE = "DC-DEV" ]; then
                /bin/bash $SCRIPT_LOCATION/scripts/common/start-scripts/tmux-start-dev.sh
            fi
            if [ $PROFILE = "DC-PROD" ]; then
                /bin/bash $SCRIPT_LOCATION/scripts/common/start-scripts/tmux-start-prod.sh
            fi
        fi
...
if [ $MODE = "--restore-data" ]; then
    if [ $PROFILE = "LOCAL" ]; then
        /bin/bash $SCRIPT_LOCATION/infra/compose/prod/on-server/backups/backup-utility.sh --restore-data-to-vols $2
    else
        echo "$MODE allowed outside Docker only."
    fi
fi
if [ $MODE = "--restore-models" ]; then
    if [ $PROFILE = "LOCAL" ]; then
        /bin/bash $SCRIPT_LOCATION/infra/compose/prod/on-server/backups/backup-utility.sh --restore-models $2
    else
        echo "$MODE allowed outside Docker only."
    fi
fi
