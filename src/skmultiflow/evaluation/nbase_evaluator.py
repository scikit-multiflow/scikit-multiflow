...
from skmultiflow.data.nbase_stream import NStream
...
class NStreamEvaluator(BaseSKMObject, metaclass=ABCMeta):
    ...
    @abstractmethod
    def partial_fit(self, t, X, y, classes=None, sample_weight=None):
    ...
    def _check_configuration(self):
        ...
        if isinstance(self.stream, NStream):
        ...
        if self._output_type == constants.SINGLE_OUTPUT:
            ...
            if evaluation_metrics.union(regression_metrics) == regression_metrics:
    ...
    def _check_progress(self, total_timestamps):
    ...
    def _init_metrics(self):
        ...
        print("nbase_evaluator.py # data_buffer #1: > {}".format(self._data_buffer))
        self._data_buffer = EvaluationDataBuffer(data_dict=self._data_dict)
        print("nbase_evaluator.py # data_buffer #2: > {}".format(self._data_buffer))

    def _update_metrics(self):
        print("nbase_evaluator.py # data_buffer #3: > {}".format(self._data_buffer))
        ...
        sample_id = self.global_timestamp_count_count + shift
    ...
    def _update_outputs(self, sample_id):
        print("nbase_evaluator.py # data_buffer #4: > {}".format(self._data_buffer))
    ...
    def _update_file(self):
        print("nbase_evaluator.py # data_buffer #5: > {}".format(self._data_buffer))
        if self.output_file is not None:
    ...
    def evaluation_summary(self):
        print("self.global_sample_count: {}".format(self.global_sample_count))# TODO delete
        if(self.global_sample_count>0):
            print("nbase_evaluator.py # data_buffer #6: > {}".format(self._data_buffer))
            ...
            for i in range(self.n_models):
                ...
                if constants.MAE in self.metrics:
                    print("nbase_evaluator.py # data_buffer #8a: > {}".format(self._data_buffer))
                    print("nbase_evaluator.py # data_buffer #8b: > {}".format(self._data_buffer.get_data(metric_id=constants.MAE, data_id=constants.MEAN)))
                    print('{} - MAE          : {:4f}'.format(
                        self.model_names[i],
