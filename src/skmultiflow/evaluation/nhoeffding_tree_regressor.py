...
def NRegressionHoeffdingTree(max_byte_size=33554432, memory_estimate_period=1000000,
    ...
    return NHoeffdingTreeRegressor(max_byte_size=max_byte_size,
...
class NHoeffdingTreeRegressor(RegressorMixin, HoeffdingTreeClassifier):
    ...
    def partial_fit(self, t, X, y, sample_weight=None):
        ...
        * If the tree is already initialized, find the corresponding leaf for the instance and update the leaf node
          statistics.
        * If growth is allowed and timestamp changed, then attempt to split.
        Parameters
        ----------
        t: numpy.ndarray of shape (n_samples, 1) indicating timestamps
        ...
        if y is not None:
            ...
            for i in range(row_cnt):
                if sample_weight[i] != 0.0:
                    ...
                    self._partial_fit(t[i], X[i], y[i], sample_weight[i])

    def _partial_fit(self, t, X, y, sample_weight):
        ...
        if isinstance(leaf_node, LearningNode):
            ...
            if self._growth_allowed and isinstance(learning_node, ActiveLearningNode):
                ...
                if t > self._last_t:
                    self._last_t = t
                    # TODO review this!: shall we take into account if weight_diff >= self.grace_period:
