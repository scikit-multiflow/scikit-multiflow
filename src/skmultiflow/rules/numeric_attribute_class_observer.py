...
class GaussianNumericAttributeClassObserver(AttributeClassObserver):
    ...
    def observe_attribute_class(self, att_val, class_val, weight):
        if att_val is None:
            ...
            try:
                ...
                self._max_value_observed_per_class = dict(
                    sorted(self._max_value_observed_per_class.items()))
                self._min_value_observed_per_class = dict(
                    sorted(self._min_value_observed_per_class.items()))
    ...
    def get_best_evaluated_split_suggestion(self, criterion, pre_split_dist, att_idx,
                                            class_idx=None):
        ...
        for expand_value in suggested_split_values:
            ...
            if best_suggestion is None or merit > best_suggestion.merit:
                if criterion.best_idx == 0:
                    ...
                    symbol = ">"
                best_suggestion = AttributeExpandSuggestion(att_idx, expand_value, symbol,
    ...
    def get_class_dists_from_binary_split(self, split_value):
        ...
        for k, estimator in self._att_val_dist_per_class.items():
            if split_value < self._min_value_observed_per_class[k]:
                ...
                weight_dist = estimator.estimated_weight_lessthan_equalto_greaterthan_value(
                    split_value)
