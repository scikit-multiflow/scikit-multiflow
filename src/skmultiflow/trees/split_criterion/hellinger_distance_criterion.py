...
class HellingerDistanceCriterion(SplitCriterion):
    ...
    The Hellinger distance is a measure of distributional divergence. It is used as
    the splitting criterion [1]_ on decision trees to to address the imbalanced data problem.
    ...
    def compute_hellinger(dist):
        ...
        hellinger = (np.sqrt(left_branch_negative / total_negative) - np.sqrt(
            left_branch_positive / total_positive)) ** 2 + (
            np.sqrt(right_branch_negative / total_negative) - np.sqrt(
                right_branch_positive / total_positive)) ** 2
