...
class GaussianEstimator(object):
    ...
    def probability_density(self, value):
        ...
        if self._weight_sum > 0.0:
            ...
            if std_dev > 0.0:
                diff = value - mean
                return (1.0 / (self._NORMAL_CONSTANT * std_dev)) * math.exp(
                    -(diff * diff / (2.0 * std_dev * std_dev)))
            if value == mean:
                return 1.0
        return 0.0
    ...
    def estimated_weight_lessthan_equalto_greaterthan_value(self, value):
        ...
        mean = self.get_mean()
        if std_dev > 0.0:
            lessthan_weight = normal_probability(
                (value - mean) / std_dev) * self._weight_sum - equalto_weight
        else:
            if value < mean:
