...
def demo():
    ...
    influence_on_positive = [[[] for _ in range(n_time_periods)] for _ in range(3)]
    influence_on_negative = [[[] for _ in range(n_time_periods)] for _ in range(3)]
    ...
    for i in range(runs):
        component_pos = RandomRBFGenerator(model_random_state=i, sample_random_state=50,
                                           n_classes=2, n_features=1, n_centroids=1, class_weights=[0, 1])
        component_neg = RandomRBFGenerator(model_random_state=i, sample_random_state=51,
                                           n_classes=2, n_features=1, n_centroids=1, class_weights=[1, 0])
        component_pos_drift = RandomRBFGeneratorDrift(model_random_state=101, sample_random_state=50,
                                                      n_centroids=1, num_drift_centroids=1,
                                                      change_speed=(1 / runs) * (i + 1),
                                                      class_weights=[0, 1])
        component_neg_drift = RandomRBFGeneratorDrift(model_random_state=101, sample_random_state=51,
        ...
        if result[1] == 0:
            if result[7] is not None:
                if result[7] < 0.05:
                    idx_pos.append(run)
            time = result[2]
            influence_pos[time].append(result[8])
            abs_mean_pos.append(result[7])
            pos.add_row(result)
        ...
        if result[1] == 0:
            if result[7] is not None:
                if result[7] < 0.05:
                    idx_neg.append(run)
            time = result[2]
            influence_neg[time].append(result[8])
            abs_mean_neg.append(result[7])
            neg.add_row(result)
