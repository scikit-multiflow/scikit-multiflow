...
def OnlineSMOTEBagging(base_estimator=KNNADWINClassifier(), n_estimators=10, sampling_rate=1,
                       drift_detection=True, random_state=None):  # pragma: no cover
    warnings.warn("'OnlineSMOTEBagging' has been renamed to 'OnlineSMOTEBaggingClassifier'"
            ...
            try:
                for i in range(self.actual_n_estimators):
                    ...
                    if len(partial_proba[0]) > max(self.classes) + 1:
                        raise ValueError("The number of classes in the base learner is "
                                         "larger than in the ensemble.")
