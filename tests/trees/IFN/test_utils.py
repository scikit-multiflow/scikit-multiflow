...
def test_split_data_to_two_intervals(interval, T, min_value, max_value, expected_left_size, expected_right_size,
                                     expected_y_values):
...
def test_find_split_position(positions, value, expected):
...
def test_drop_records(X, y, index, value, expected_X, expected_y):
...
def test_create_attribute_node(X, y, chosen_attribute_index, attribute_value, curr_node_index, prev_node_index,
                               expected_X_length, expected_y_length):
...
def test_convert_numeric_values(chosen_split_points, chosen_attribute, X, expected):
...
def test_calculate_second_best_attribute_of_last_layer(mi_dict: dict, expected_index, expected_mi):
